...
int main(int argc, char *argv[])
{
    ...
    if(argc == 4)
    {
        memPtr[0] = atoi(argv[3]);
    }

    int *varStack = memPtr+1048576/8;                // Variables on Stack Head
    int *stackHead = memPtr+(1048576/8 + 4*16);       // Execution Stack Head
    int *PC = memPtr+1024/8;                         // Program Counter
    int *CP = memPtr+256/8;                          // Constant Pool Cunter
